...
import com.triton.nanny.petlover.PetVendorOrderDetailsActivity;
import com.triton.nanny.petlover.PetVendorTrackOrderActivity;
import com.triton.nanny.responsepojo.PetVendorOrderResponse;
    ...
    private void initLayoutOne(ViewHolderOne holder, final int position) {
        ...
        if (newOrderResponseList.get(position).getProduct_price() != 0 && newOrderResponseList.get(position).getProduct_quantity() != 0) {
            if (newOrderResponseList.get(position).getProduct_quantity() == 1) {
                holder.txt_products_price.setText("RM  " + newOrderResponseList.get(position).getProduct_price() + " (" + newOrderResponseList.get(position).getProduct_quantity() + " item )");
            } else {
                holder.txt_products_price.setText("RM  " + newOrderResponseList.get(position).getProduct_price() + " (" + newOrderResponseList.get(position).getProduct_quantity() + " items )");

            }
        }
        else { if (newOrderResponseList.get(position).getProduct_quantity() == 1) {
                holder.txt_products_price.setText("RM  " + 0 + " (" + newOrderResponseList.get(position).getProduct_quantity() + " item )");
            }
            else {
                holder.txt_products_price.setText("RM  " + 0 + " (" + newOrderResponseList.get(position).getProduct_quantity() + " items )");
